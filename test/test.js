...
describe('Default Generator', () => {
    ...
    describe('newToken(clientId, options)', () => {
        ...
        it('Should accept custom duration.', async () => {
            let {token, record} = await generator.newToken('client1', { duration: {minutes: 30} })
            let durationMillis = record.expires.toMillis() - record.issued.toMillis()
            assert.equal(durationMillis, (30 * 60 * 1000))
        })
    ...
    describe('verifyToken(token, options)', () => {
        describe('If successful:', () => {
            ...
            it('Payload should retain custom claims.', () => {
        ...
        describe('If failed:', () => {
            ...
            it('Should fail for old tokens.', async () =>{
